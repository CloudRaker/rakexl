...
description: Complete reference for all Rakexl functions and transforms
---

# Function Reference

Rakexl provides over 80 built-in functions and transforms organized into the following categories:
...
async function generateMDXDocs(): Promise<void> {
  ...
  console.log("🏗️  Generating MD documentation...");
  ...
  fs.writeFileSync(path.join(outputDir, "index.md"), mainIndex);
  ...
  for (const [categoryKey, data] of Object.entries(categorized)) {
    ...
    fs.writeFileSync(path.join(categoryDir, "index.md"), categoryIndex);

    // Generate individual function pages
    for (const item of data.items) {
      const functionMDX = generateFunctionMDX(item);
      fs.writeFileSync(
        path.join(categoryDir, `${item.label}.md`),
        functionMDX
      );
      totalGenerated++;
    }
  ...
  console.log(
    `\n🎉 Successfully generated ${totalGenerated} MD pages in ${outputDir}/`
  );
  console.log(
    `📁 Categories created: ${
  ...
  // Generate VitePress sidebar JSON for the reference section
  const sidebarReference = [
    ...
    ...Object.entries(categorized)
      .filter(([_, data]) => data.items.length > 0)
      .map(([key, data]) => {
        // Deduplicate items by label (functions and transforms share the same page)
        ...
        return {
          ...
          collapsed: true,
          items: [
            { text: "Overview", link: `/reference/${key}/` },
            ...uniqueItems.map((item) => ({
              text: item.label,
              link: `/reference/${key}/${item.label}`,
            })),
          ],
        };
      }),
  ...
  const sidebarPath = path.join("./docs/.vitepress", "sidebar-reference.json");
  fs.writeFileSync(sidebarPath, JSON.stringify(sidebarReference, null, 2));
  console.log("✅ Generated sidebar-reference.json");
}
